...
const BIOMETRIC_API = process.env.BIOMETRIC_API_URL || "http://localhost:8000";

// Proxy identification faciale vers backend Python
app.post("/api/biometric/identify", async (req, res) => {
  try {
    const response = await fetch(`${BIOMETRIC_API}/identify`, {
      method: "POST",
    ...
    return res.json({
      success: false,
      mode: "simulation",
      result: "BIOMETRIC_OFFLINE",
      message: "Module biométrique hors ligne — mode simulation actif",
    });
  }
});

// Proxy enregistrement visage
app.post("/api/biometric/register", async (req, res) => {
  try {
    const response = await fetch(`${BIOMETRIC_API}/register`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(req.body),
    });
    ...
    return res.json({ success: false, error: err.message, mode: "simulation" });
  }
});

// Proxy statistiques base biométrique
app.get("/api/biometric/stats", async (req, res) => {
  try {
    const response = await fetch(`${BIOMETRIC_API}/stats`);
    const data = await response.json();
    return res.json(data);
  } catch (err: any) {
    return res.json({
      success: true,
      mode: "simulation",
      stats: { total_faces: 0, status: "OFFLINE", message: "Backend Python non connecté" },
    });
  }
});

// ═══════════════════════════════════════════════════════
//   BAND-6G SENTINEL — DÉTECTIONS (ALGERIE SENTINEL)
// ═══════════════════════════════════════════════════════

const DETECTION_TYPES = [
  { ico:"🛸", type:"ALSAT-2B IMAGE HR", priority:1, cat:"satellite", color:"#ffd700", sources:["ALSAT-2B","ASAL"] },
  { ico:"🚁", type:"DRONE TB2 DETECTION", priority:1, cat:"drone", color:"#00ff88", sources:["TB2-BAYRAKTAR","FAA"] },
  { ico:"🚁", type:"DRONE CH-5 FRAPPE", priority:1, cat:"drone", color:"#00ff88", sources:["CAIG-CH5","ANP"] },
  { ico:"📡", type:"RADAR S-300 ACTIVATION", priority:1, cat:"radar", color:"#ff8c00", sources:["S-300PMU2","DEF-AA"] },
  { ico:"📡", type:"RADAR FRONTALIER OTH", priority:2, cat:"radar", color:"#ff8c00", sources:["OTH-SAHARA","GGF"] },
  { ico:"💣", type:"ACTIVITE TERRORISTE", priority:1, cat:"terrorisme", color:"#e63a2e", sources:["DSS","HUMINT","GIS"] },
  { ico:"💣", type:"CELLULE ACTIVE AQIM", priority:1, cat:"terrorisme", color:"#e63a2e", sources:["DSS-CT","SIGINT"] },
  { ico:"💊", type:"NARCOTRAFIC DETECTE", priority:2, cat:"narco", color:"#9b59b6", sources:["DCSA","DRONE"] },
  { ico:"🔫", type:"TRAFIC ARMES DETECTE", priority:1, cat:"armes", color:"#e74c3c", sources:["GGF","HUMINT","DSS"] },
  { ico:"🚢", type:"EMBARCATION SUSPECTE", priority:2, cat:"maritime", color:"#1a9fff", sources:["RADAR-NAVAL","MARINE"] },
  { ico:"🚢", type:"GO-FAST DETECTE", priority:1, cat:"maritime", color:"#1a9fff", sources:["MARINE","HELICOPTER"] },
  { ico:"👥", type:"MIGRATION IRREGULIERE", priority:3, cat:"migration", color:"#f39c12", sources:["CAPTEUR-IR","SATELLITE"] },
  { ico:"💻", type:"CYBERATTAQUE DETECTEE", priority:1, cat:"cyber", color:"#00ced1", sources:["ANCS","COMCYBER"] },
  { ico:"📦", type:"CONVOI CLANDESTIN", priority:2, cat:"contrebande", color:"#e67e22", sources:["DRONE","SATELLITE","GGF"] },
  { ico:"✈️", type:"VIOLATION ESPACE AERIEN", priority:1, cat:"aviation", color:"#00d4ff", sources:["AADN","S-300"] },
  { ico:"🔥", type:"INCENDIE DETECTE", priority:2, cat:"incendie", color:"#ff4500", sources:["SATELLITE-IR","CRAAG"] },
  { ico:"☢️", type:"ALERTE NRBC DETECTEE", priority:1, cat:"nrbc", color:"#ff1a1a", sources:["NRBC-ANP","UMD"] },
  { ico:"🕵️", type:"ESPIONNAGE DETECTE", priority:1, cat:"espionnage", color:"#8e44ad", sources:["DSS-CE","SIGINT"] },
];

const DZ_ZONES = [
  { name:"Alger", lat:36.737, lon:3.086 },
  { name:"Oran", lat:35.697, lon:-0.633 },
  { name:"Constantine", lat:36.365, lon:6.615 },
  { name:"Annaba", lat:36.902, lon:7.755 },
  { name:"Tamanrasset", lat:22.785, lon:5.523 },
  { name:"Bechar", lat:31.617, lon:-2.216 },
  { name:"Tindouf", lat:27.674, lon:-8.146 },
  { name:"Ouargla", lat:31.949, lon:5.325 },
  { name:"Tlemcen", lat:34.880, lon:-1.316 },
  { name:"Batna", lat:35.556, lon:6.174 },
  { name:"Setif", lat:36.190, lon:5.408 },
  { name:"Ghardaia", lat:32.490, lon:3.674 },
  { name:"Frontiere Mali", lat:19.3, lon:2.0 },
  { name:"Frontiere Niger", lat:21.5, lon:8.5 },
  { name:"Frontiere Libye", lat:28.5, lon:9.2 },
  { name:"Frontiere Tunisie", lat:36.2, lon:8.5 },
  { name:"Frontiere Maroc", lat:35.1, lon:-2.0 },
  { name:"Mediterranee", lat:37.2, lon:3.5 },
  { name:"Mascara", lat:35.396, lon:0.141 },
  { name:"Mostaganem", lat:35.931, lon:0.089 },
];

function generateDetections(n = 20) {
  const result = [];
  for (let i = 0; i < n; i++) {
    const det = DETECTION_TYPES[Math.floor(Math.random() * DETECTION_TYPES.length)];
    const zone = DZ_ZONES[Math.floor(Math.random() * DZ_ZONES.length)];
    const src = det.sources[Math.floor(Math.random() * det.sources.length)];
    const conf = Math.floor(Math.random() * 28) + 68;
    const niv = det.priority === 1 ? Math.floor(Math.random() * 3) + 8 :
                det.priority === 2 ? Math.floor(Math.random() * 3) + 5 :
    ...
    result.push({
      ...
      source: src,
      wilaya: zone.name,
      lat: +(zone.lat + off()).toFixed(5),
      lon: +(zone.lon + off()).toFixed(5),
      confidence: conf,
      threatLevel: niv,
      critical: niv >= 8,
      timestamp: new Date(Date.now() - Math.random() * 1800000).toISOString(),
      classification: niv >= 8 ? "TRES SECRET" : niv >= 5 ? "SECRET" : "CONFIDENTIEL",
      status: niv >= 8 ? "ALERTE ROUGE" : niv >= 5 ? "ALERTE ORANGE" : "SURVEILLANCE",
    });
  }
  return result;
}

app.get("/api/sentinel/detections", (req, res) => {
  const n = Math.min(parseInt((req.query.n as string) || "20"), 50);
  res.json({
    success: true,
    total: n,
    timestamp: new Date().toISOString(),
    source: "BAND-6G-SENTINEL",
    detections: generateDetections(n),
  });
});
...
app.get("/api/sentinel/stats", (req, res) => {
  res.json({
    ...
    stats: {
      ...
      zones_monitored: DZ_ZONES.length,
      satellites_active: 3,
      drones_active: Math.floor(Math.random() * 8) + 2,
