...
export default function SentinelDetections() {
  ...
  // Init map only when view=carte AND leaflet ready
  useEffect(() => {
    if (!mapReady || view !== "carte" || mapRef.current || !mapDivRef.current) return;
    const L = (window as any).L;
    const map = L.map(mapDivRef.current, { center: [28.0, 3.0], zoom: 5 });
    L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
      attribution: "© OSM", opacity: 0.4
    }).addTo(map);
    mapDivRef.current.style.filter = "invert(1) hue-rotate(180deg) brightness(0.85) saturate(1.4)";
    mapRef.current = map;
    // GPS
    navigator.geolocation?.getCurrentPosition(pos => {
      ...
      const icon = L.divIcon({
        html: `<div style="width:16px;height:16px;background:#00ff88;border-radius:50%;border:3px solid #fff;box-shadow:0 0 14px #00ff88"></div>`,
        className:"", iconAnchor:[8,8]
      });
      L.marker([lat,lon],{icon}).addTo(map).bindPopup("<b>📍 VOTRE POSITION GPS</b>").openPopup();
      map.setView([lat,lon],7);
    });
    // Add existing markers
    if (detections.length > 0) addMarkers(map, detections);
  }, [mapReady, view]);

  function addMarkers(map: any, dets: Detection[]) {
    const L = (window as any).L;
    if (!L) return;
    markersRef.current.forEach(m => m.remove());
    markersRef.current = [];
    const toShow = filter === "all" ? dets : dets.filter(d => d.cat === filter);
    toShow.forEach(det => {
      const color = det.critical ? "#ff2222" : det.threatLevel >= 5 ? "#ff9500" : det.color;
      const sz = det.critical ? 14 : 10;
      const icon = L.divIcon({
        html: `<div style="width:${sz}px;height:${sz}px;background:${color};border-radius:50%;border:2px solid rgba(255,255,255,0.5);box-shadow:0 0 8px ${color};cursor:pointer"></div>`,
        className:"", iconAnchor:[sz/2,sz/2]
      });
      const m = L.marker([det.lat, det.lon], {icon}).addTo(map);
      m.bindPopup(`<div style="font-family:monospace;font-size:11px;min-width:170px">
        <b style="color:${color}">${det.ico} ${det.type}</b><br/>
        Zone: <b>${det.wilaya}</b><br/>
        Source: ${det.source}<br/>
        Menace: <b>${det.threatLevel}/10</b><br/>
        Confiance: ${det.confidence}%<br/>
        <span style="color:${det.critical?'#ff4444':'#ff9500'}">${det.status}</span>
      </div>`);
      m.on("click", () => setSelected(det));
  ...
  const fetchDetections = useCallback(async () => {
    try {
      ...
      if (d1.success && d1.detections?.length > 0) {
        ...
        if (mapRef.current) addMarkers(mapRef.current, d1.detections);
      }
  ...
  useEffect(() => {
    if (mapRef.current && detections.length > 0) addMarkers(mapRef.current, detections);
  }, [filter]);
  ...
  return (
    <div style={{fontFamily:"'Rajdhani',sans-serif", color:"#cce0cc", padding:"12px"}}>
      ...
      {/* Toolbar */}
      <div style={{display:"flex",gap:"5px",alignItems:"center",marginBottom:"8px",flexWrap:"wrap"}}>
        ...
        <div style={{marginLeft:"auto",display:"flex",gap:"5px",alignItems:"center"}}>
          ...
          <button onClick={()=>{
            setView(v=>v==="carte"?"liste":"carte");
          }} style={{
            padding:"4px 12px",fontSize:"8px",fontFamily:"'Orbitron',monospace",
            background:"rgba(0,80,120,0.2)",border:"1px solid #00e5cc",color:"#00e5cc",cursor:"pointer"
      ...
      {loading ? (
        <div style={{textAlign:"center",padding:"80px",color:"#00a855",fontFamily:"'Orbitron',monospace",fontSize:"11px",letterSpacing:"4px"}}>
          <div style={{fontSize:"40px",marginBottom:"12px"}}>🛡️</div>SCAN NATIONAL EN COURS...
        </div>
      ) : (
        <>
          {/* CARTE */}
          {view==="carte" && (
            <div style={{position:"relative"}}>
              <div ref={mapDivRef} style={{width:"100%",height:"480px",background:"#0a1a0e",border:"1px solid rgba(0,98,51,0.4)",borderRadius:"4px"}} />
              <div style={{position:"absolute",top:"8px",left:"8px",zIndex:1000,
                background:"rgba(2,8,3,0.92)",border:"1px solid rgba(0,98,51,0.4)",padding:"8px 12px",fontSize:"9px",fontFamily:"'Orbitron',monospace",pointerEvents:"none"}}>
                <div style={{color:"#00a855",marginBottom:"3px"}}>🇩🇿 ALGÉRIE SENTINEL</div>
                <div style={{color:"rgba(200,220,200,0.5)"}}>{filtered.length} détections actives</div>
                {userPos && <div style={{color:"#00ff88",marginTop:"2px"}}>📍 GPS ACTIF — {userPos.lat.toFixed(2)}°N</div>}
              </div>
              <div style={{position:"absolute",bottom:"8px",right:"8px",zIndex:1000,
                background:"rgba(2,8,3,0.9)",border:"1px solid rgba(0,98,51,0.3)",padding:"8px 10px"}}>
                {[{c:"#ff2222",l:"CRITIQUE"},{c:"#ff9500",l:"ALERTE"},{c:"#00a855",l:"SURVEILLANCE"}].map(item=>(
                  <div key={item.l} style={{display:"flex",alignItems:"center",gap:"6px",marginBottom:"3px"}}>
                    <div style={{width:"8px",height:"8px",borderRadius:"50%",background:item.c,boxShadow:`0 0 5px ${item.c}`}} />
          ...
          {view==="liste" && (
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:"5px",maxHeight:"520px",overflowY:"auto"}}>
              {filtered.length===0 ? (
                <div style={{gridColumn:"1/-1",textAlign:"center",padding:"40px",color:"rgba(0,168,85,0.4)",fontFamily:"'Orbitron',monospace",fontSize:"10px"}}>
                ...
                <div key={det.id} onClick={()=>setSelected(det)} style={{
                  ...
                  {det.critical && <div style={{float:"right",fontSize:"6px",fontFamily:"'Orbitron',monospace",color:"#ff4444",animation:"blink 1s infinite"}}>● CRITIQUE</div>}
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"3px",fontSize:"8px",marginBottom:"5px"}}>
                    ...
                    <div style={{fontSize:"7px",color:"rgba(200,220,200,0.25)"}}>{new Date(det.timestamp).toLocaleTimeString("fr-FR")}</div>
                  </div>
          ...
          {/* Detail Panel */}
          {selected && (
            <div style={{marginTop:"10px",padding:"12px",background:"rgba(0,15,8,0.98)",border:`1px solid ${selected.color}66`}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"8px"}}>
                <div style={{fontFamily:"'Orbitron',monospace",fontSize:"11px",color:selected.color,fontWeight:700}}>
                  {selected.ico} {selected.type}
                </div>
                <button onClick={()=>setSelected(null)} style={{background:"transparent",border:"none",color:"rgba(200,220,200,0.4)",fontSize:"16px",cursor:"pointer"}}>✕</button>
              </div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:"8px",fontSize:"10px"}}>
                {[
                  ...
                  {l:"MENACE",v:`${selected.threatLevel}/10`,c:selected.threatLevel>=8?"#ff4444":selected.threatLevel>=5?"#ff9500":"#00a855"},
                  {l:"CLASSIFICATION",v:selected.classification,c:"#ffd700"},
                  {l:"STATUT",v:selected.status,c:selected.critical?"#ff4444":"#ff9500"},
                  {l:"HEURE",v:new Date(selected.timestamp).toLocaleTimeString("fr-FR"),c:"rgba(200,220,200,0.5)"},
                ].map(item=>(
                  <div key={item.l}>
              ...
              <button onClick={()=>{
                if(mapRef.current){ setView("carte"); setTimeout(()=>mapRef.current.setView([selected.lat,selected.lon],10),300); }
              }} style={{
                marginTop:"10px",padding:"5px 14px",fontSize:"8px",fontFamily:"'Orbitron',monospace",letterSpacing:"2px",
                background:"rgba(0,98,51,0.2)",border:"1px solid #00a855",color:"#00ff88",cursor:"pointer"
