...
import SentinelDetections from "./components/SentinelDetections";
...
export default function App() {
  ...
  const [activeLayerTab, setActiveLayerTab] = useState<"handset" | "gateway" | "voip" | "map" | "dz-int" | "sentinel" | "biometric">("handset");
  ...
  return (
    <div className="min-h-screen relative overflow-hidden bg-zinc-950 text-white p-4 md:p-8 select-none flex flex-col justify-between">
      ...
      <div className="max-w-7xl mx-auto w-full mb-6 grid grid-cols-2 md:grid-cols-7 gap-3">
        <button
        ...
        <button
          onClick={() => { playKeyClick(); setActiveLayerTab("sentinel"); }}
          className={`p-3 rounded-2xl border font-mono font-bold text-xs flex flex-col sm:flex-row items-center gap-2.5 cursor-pointer transition-all ${
            activeLayerTab === "sentinel"
              ? "bg-green-950/40 text-green-400 border-green-800 shadow-[0_0_12px_rgba(0,255,100,0.15)]"
              : "bg-zinc-900/30 text-zinc-500 hover:text-zinc-400 border-zinc-900/60 hover:border-zinc-800/80"
          }`}
        >
          <span style={{fontSize:"18px"}}>🛡️</span>
          <div className="text-left leading-tight">
            <div className="text-[11px]">SENTINEL DZ</div>
            <div className="text-[9px] font-normal uppercase opacity-75">Détections Nationales</div>
          </div>
        </button>

        <button
          onClick={() => { playKeyClick(); setActiveLayerTab("biometric"); }}
          className={`p-3 rounded-2xl border font-mono font-bold text-xs flex flex-col sm:flex-row items-center gap-2.5 cursor-pointer transition-all ${
            activeLayerTab === "biometric"
              ? "bg-purple-950/40 text-purple-400 border-purple-800 shadow-[0_0_12px_rgba(168,85,247,0.15)]"
              : "bg-zinc-900/30 text-zinc-500 hover:text-zinc-400 border-zinc-900/60 hover:border-zinc-800/80"
          }`}
        >
          <span style={{fontSize:"18px"}}>👁️</span>
          <div className="text-left leading-tight">
            <div className="text-[11px]">BIOMÉTRIE</div>
            <div className="text-[9px] font-normal uppercase opacity-75">Scan Facial Terrain</div>
          </div>
        </button>
      </div>
        ...
        <main className="max-w-7xl mx-auto w-full flex-1 mb-8">
          <div className="bg-zinc-950 rounded-3xl border border-zinc-800 shadow-xl overflow-hidden">
            <div className="p-4 border-b border-zinc-800 flex items-center gap-3">
              ...
              <div>
                <div style={{fontFamily:"'Orbitron', monospace", fontSize:"13px", fontWeight:700, color:"#00a855", letterSpacing:"3px"}}>
                  ALGÉRIE SENTINEL
                </div>
                <div style={{fontSize:"9px", color:"rgba(0,168,85,0.5)", letterSpacing:"2px"}}>
                  SYSTÈME DE DÉTECTION NATIONAL — BAND-6G INTÉGRÉ
                </div>
              </div>
            </div>
            <SentinelDetections />
      ...
      <div className="hidden xl:block absolute bottom-8 left-8 text-zinc-750 text-[11px] font-mono border-l border-zinc-900 pl-4 leading-relaxed pointer-events-none">
        <span className="text-zinc-600">FIRMWARE:</span> <span className="text-zinc-500">v6.0.0-BAND6G</span><br />
      ...
      <footer className="max-w-7xl mx-auto w-full text-center border-t border-zinc-880 pt-4 font-mono text-[10px] text-zinc-600 flex flex-col sm:flex-row justify-between items-center gap-2 z-10">
        <div>
          BAND-6G SENTINEL • Talky Walky + Biométrie + Détection Nationale • Fondé par Khedim Benyakhlef dit Beny Joe 🇩🇿
        </div>
        <div className="flex items-center gap-2">
